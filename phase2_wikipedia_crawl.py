...
ROOT='Category:Fables'
OUT='phase2_fables.json'
INDEX_HINTS=("list of", "aesop's fables", 'panchatantra', 'jataka', 'hitopadesha', 'kalila and dimna', "la fontaine's fables", 'perry index')
...
def index_links(page):
    tl=page['title'].casefold()
    if not any(h in tl for h in INDEX_HINTS): return []
    ...
    for link in code.filter_wikilinks(recursive=True):
        ...
        links.append(title)
    # only treat as a tale-level/index expansion when it actually behaves like an index
...
def main():
    ...
    while q:
        ...
        for m in members:
            if m['ns']==14:
                q.append(m['title']); cat_edges.append({'parent':cat,'child':m['title']})
            elif m['ns']==0:
                direct.append({'category':cat,'listed_pageid':m['pageid'],'listed_title':m['title']})
    ...
    pages1,fail1=fetch_pages(direct_titles); failures.extend(fail1)
    index_sources=[]; index_targets=[]
    ...
    pages2,fail2=fetch_pages([x['listed_title'] for x in index_targets]); failures.extend(fail2)
    by_pid=defaultdict(lambda:{'category_memberships':[],'index_sources':[],'listed_titles':[]}); pages={}
    for x in direct:
        p=pages1.get(x['listed_title'])
        if p:
            pages[p['pageid']]=p; d=by_pid[p['pageid']]; d['category_memberships'].append(x['category']); d['listed_titles'].append(x['listed_title'])
    for x in index_targets:
        p=pages2.get(x['listed_title'])
        if p:
            pages[p['pageid']]=p; d=by_pid[p['pageid']]; d['index_sources'].append({'source_title':x['source_title'],'source_pageid':x['source_pageid'],'source_url':x['source_url']}); d['listed_titles'].append(x['listed_title'])
    records=[]
    for pid,prov in by_pid.items():
        p=pages[pid]
        records.append({k:v for k,v in p.items() if k!='wikitext'} | {'category_memberships':sorted(set(prov['category_memberships'])),'index_sources':list({(x['source_pageid'],x['source_title'],x['source_url']):x for x in prov['index_sources']}.values()),'listed_titles':sorted(set(prov['listed_titles']))})
    ...
    print(json.dumps({k:payload[k] for k in ['category_count','direct_memberships_total','distinct_direct_titles','index_source_count','index_link_memberships_total','distinct_listed_titles_union','distinct_canonical_pageids','failures']},ensure_ascii=False))
